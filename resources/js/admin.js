...
window.Echo.join("online")
            ...
            $(".profile-card").each(function () {
                ...
                if (profileUserId == user.id) {
                    $(this).find(".user-status")
                        .html(` <div class="text-success text-small font-600-bold"><i
    ...
    .joining((user) => {
        $(`.profile-card[data-sender-id="${user.id}"]`).find(".user-status")
            .html(`<div class="text-success text-small font-600-bold"><i
            class="fas fa-circle"></i>
        Online</div>`);
    })
    .leaving((user) => {
        $(`.profile-card[data-sender-id="${user.id}"]`).find(".user-status")
            .html(` <div class="text-secondory text-small font-600-bold"><i
            class="fas fa-circle"></i>
        Offline</div>`);
    });
