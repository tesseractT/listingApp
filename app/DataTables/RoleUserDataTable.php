        ...
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                if ($query->getRoleNames()->first() !== 'Super Admin') {
                    ...
                    $delete = '<a href ="' . route('admin.role-user.destroy', $query->id) . '" class="delete-item btn btn-sm btn-danger ml-2"><i class="fas fa-trash"></a>';
                    return $edit . $delete;
                }
            })

            ->addColumn('role', function ($role) {
                return " <span class='badge badge-primary'>" . $role->getRoleNames()->first() . "</span>";
            })
