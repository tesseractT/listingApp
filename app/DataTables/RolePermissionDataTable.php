        ...
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                if ($query->name !== 'Super Admin') {
                    ...
                    $delete = '<a href ="' . route('admin.role.destroy', $query->id) . '" class="delete-item btn btn-sm btn-danger ml-2"><i class="fas fa-trash"></a>';
                    return $edit . $delete;
                }
            })
            ->addColumn('permissions', function ($query) {
                $html = '';
                if ($query->name === 'Super Admin') {
                    $html .= '<span class="badge badge-danger m-1">All Permissions Granted</span>';
                } else {
