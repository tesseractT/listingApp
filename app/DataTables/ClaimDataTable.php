    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $delete = '<a href ="' . route('admin.listing-claims.destroy', $query->id) . '" class="delete-item btn btn-sm btn-danger ml-2"><i class="fas fa-trash"></a>';
                return $delete;
            })
            ...
            ->addColumn('image', function ($query) {
                return '<img width="70" src=" ' . asset($query->listing->image) . ' " >';
            })
            ...
            ->rawColumns(['action', 'image', 'listing'])
            ->setRowId('id');
    }
